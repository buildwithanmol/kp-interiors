    ...
    return (
        <>
            ...
            <section className=' grid  grid-cols-1  md:grid-cols-2 md:grid-rows-2 gap-8 max-w-4xl  my-0 mx-auto px-[1rem] md:px-0'>
                ...
                {
                    workFlw.map((e, i) => (
                        <div key={i} className='bg-white border shadow text-[#737373] rounded-xl relative flex items-center justify-center flex-col gap-5 text-center md:py-[5vw] py-[8vw] '>
                            {e.icon}
                            <span>
                            ...
                            <p className='p-1 rounded-full bg-[#6fc3dd] text-white w-8 h-8 absolute -top-2 -left-2'>{i + 1}</p>
                        </div>
                    ))
                }
            </section>
